...
async function handle(req, env) {
  ...
  if (p === "/due") {
    ...
    const rec = JSON.parse(await env.BASELINE.get("sub:" + uid) || "null");
    if (!rec) return json({ due: [], count: 0, closed: [], done: [], skipped: [] }, env);
  ...
  if (p === "/test" && req.method === "POST") {
    ...
    return json({ ok: status >= 200 && status < 300, status }, env);
  }
